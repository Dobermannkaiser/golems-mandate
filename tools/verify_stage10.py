...
def validate_integration() -> None:
    ...
    check("Versão 2.10.1", 'config/version="2.10.1"' in project)
    check("Layout oficial aprovado permanece", "res://scripts/UIManagerVariantB.gd" in scene)
    check("Save v7", "const SAVE_VERSION: int = 7" in save)
...
def validate_stage8_system() -> None:
    ...
    check("Save v7 e caminho novo", "SAVE_VERSION: int = 7" in save and "v2_10_0_save.json" in save)
...
def validate_stage10_balance() -> None:
    ...
    check("Versão de manutenção 2.10.1", 'config/version="2.10.1"' in project)
    ...
    check("Save v7 separado", "SAVE_VERSION: int = 7" in save and "v2_10_0_save.json" in save)
    check("Conversão v6 para v7", "_migrate_v6_to_v7" in save and '"moderate"' in save)
    check("Saves antigos permanecem localizáveis", "v2_8_0_save.json" in save and "v2_7_0_save.json" in save)
